...
Webcam::Webcam() : AbstractWebcamManager()
{
    _camera                  = new QCamera();
    auto _cameraFrameGrabber = new CameraFrameGrabber();
    _camera->setViewfinder(_cameraFrameGrabber);
    //    connect(_cameraFrameGrabber, SIGNAL(frameAvailable(QImage)), this,
    //    SLOT(handleFrame(QImage)));
    _camera->start();
}

void Webcam::startSlot()
{
    _camera->start();
}
