...
static HKEY toHkey(const RegKey::HiveKeys &hive)
{
    switch (hive) {
...
QMap<QString, QString> RegKey::EnumValues() const
{
    _ASSERTE(IsValid());
...
//        };
    ...
    QMap<QString, QString> valueInfo;
...
RegKey::RegKey(const HiveKeys hKey)
    : m_hKey{ toHkey(hKey) }
{}

RegKey::RegKey(const HiveKeys &hKeyParent, const QString &subKey)
{
    Create(hKeyParent, subKey);
}

RegKey::RegKey(const HiveKeys hKeyParent, const QString &subKey, REGSAM desiredAccess)
{
...
bool RegKey::Create(const HiveKeys hKeyParent, const QString &subKey, const REGSAM desiredAccess)
{
...
bool RegKey::Create(const HiveKeys hKeyParent, const QString &subKey, const REGSAM desiredAccess, const DWORD options, SECURITY_ATTRIBUTES * const securityAttributes, DWORD * const disposition)
{
    HKEY hKey{ nullptr };
...
bool RegKey::Open(const HiveKeys hKeyParent, const QString &subKey, const REGSAM desiredAccess)
{
    HKEY hKey{ nullptr };
